...
export const highlights: Record<string, HighlightEntry> = {
  ...
  Maqbool61:            { name: 'Maqbool Ahmed',        highlight: 'Fixed /stats overcounting with anchored markers, cost gate for unsupported IDEs in accumulateEvent, chmod 600 hardening on audit.log, aider-watcher reset on file truncation/rotation, hybrid debounce with extension filtering in codebuddy-adapter, Continue.dev MCP registration via standalone YAML block files in ~/.continue/mcpServers/, extracted audit-log.ts module with recursive secret redaction (JWT, hex, base64) and 5 MB rotation from egc-guardian, and egc replay with session playback and timeline scrubbing', since: 'Jun 29, 2026', prs: 8, badges: ['bugHunter', 'reliability', 'newFeature', 'multiFeature'], additions: 1958 },
  ...
  fuentes71:            { name: 'Matheus Fuentes',        highlight: 'Fixed a native Node.js crash (libuv uv_async.c assertion) on Windows when the state store fails to initialize, with idempotent DB close and graceful process exit', since: 'Jul 3, 2026', prs: 1, badges: ['bugHunter', 'crossPlatform'], additions: 17 },
};
