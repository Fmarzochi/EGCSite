...
export const highlights: Record<string, HighlightEntry> = {
  ...
  Maqbool61:            { name: 'Maqbool Ahmed',        highlight: 'Fixed /stats overcounting, cost gate for unsupported IDEs, chmod 600 hardening, aider-watcher reset on file truncation, hybrid debounce with extension filtering, Continue.dev MCP registration, audit-log.ts with recursive secret redaction and 5 MB rotation, egc replay with timeline scrubbing, HMAC-SHA256 integrity check on state files with per-user key in ~/.egc/integrity.key, and native Zed IDE integration via template-driven mcp-register.js', since: 'Jun 29, 2026', prs: 10, badges: ['bugHunter', 'reliability', 'newFeature', 'multiFeature', 'security'], additions: 2401 },
