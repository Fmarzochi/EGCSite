...
export const highlights: Record<string, HighlightEntry> = {
  ...
  'hanu-14':            { name: 'Mohammed Hanan',         highlight: 'Preserved multi-byte UTF-8 characters split across TCP chunks in the dashboard POST /event handler, buffering the raw bytes before decoding so multibyte payloads survive chunk boundaries, then added an explicit per-client HTTP timeout across every LLM provider', since: 'Jul 21, 2026', prs: 2, commits: 2, badges: ['bugHunter', 'reliability'], additions: 588 },
};
