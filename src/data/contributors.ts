...
export const highlights: Record<string, HighlightEntry> = {
  ...
  Maqbool61:            { name: 'Maqbool Ahmed',        highlight: 'Fixed /stats overcounting, cost gate for unsupported IDEs, chmod 600 hardening, aider-watcher reset on file truncation, hybrid debounce with extension filtering, Continue.dev MCP registration, audit-log.ts with recursive secret redaction and 5 MB rotation, egc replay with timeline scrubbing, HMAC-SHA256 integrity check on state files, native Zed IDE integration via template-driven mcp-register.js, and AES-256-GCM encryption for .md state files at rest with backward-compatible migration', since: 'Jun 29, 2026', prs: 11, badges: ['bugHunter', 'reliability', 'newFeature', 'multiFeature', 'security'], additions: 2547 },
  ...
  developmentwithparth1311: { name: 'Parth',            highlight: 'Scoped rate limiter per project path in egc-guardian; capped POST /event body at 256 KB to prevent memory exhaustion DoS; strict CLI flag validation in egc replay rejecting unrecognized options', since: 'Jun 30, 2026', prs: 3, badges: ['newFeature', 'reliability', 'bugHunter', 'codeQuality'], additions: 175 },
  ...
  fuentes71:            { name: 'Matheus Fuentes',        highlight: 'Fixed a native Node.js crash (libuv uv_async.c assertion) on Windows: idempotent DB close, BOM-safe JSON parsing, ping() try/catch, graceful process exit, plus stress tests for db-adapter, state-store and telemetry', since: 'Jul 3, 2026', prs: 1, badges: ['bugHunter', 'crossPlatform'], additions: 334 },
};
